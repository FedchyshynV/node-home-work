...
export const login = async (req: express.Request, res: express.Response) => {
  ...
  try {
    logger.info(`service: UserService, method: getUser, params: { login: ${login}, password: ${password}}`)
    const user: IUser = await userServiceInstance.getUser(login, password);
    
    if (!user) {
      res.status(MESSAGE_TYPE.UNAUTHORIZED).send({ success: false, message: 'Bad username/password combination.' });
    } else {
      const payload = {sub: user.id, title: user.login};
      const token = jwt.sign(payload, TOKEN.ACCESS_TOKEN_SECRET, { expiresIn: 12000 });

      res.send(token);
    }

  } catch (e) {
    logger.error(`service: UserService, method: getUser, params: { login: ${login}, password: ${password}}, message: Some error occurred while getting the user.`);
  }
};